      ...
      <section>
        ...
        {familias.length === 0 ? (
          ...
          <div className="space-y-3">
            {familias.map(f => (
              <div key={f.id} className="bg-[#161b22] border border-[#30363d] rounded-2xl p-4 flex items-center justify-between gap-4">
                <div className="flex items-center gap-3">
                  <span className="text-2xl">{f.avatar ?? '👤'}</span>
                  <div>
                    <div className="font-bold text-[#e6edf3]">{f.nombre}</div>
                    <div className="text-xs text-[#8b949e]">{f.email}</div>
                    <div className="text-xs text-[#484f58] mt-0.5">Registrado {f.creado}</div>
                  </div>
                </div>
